...
#let project(
  ...
  font_roman: ("Times New Roman", "Times", "Nimbus Roman No9 L Regular"),
  body,
) = {
  ...
  set text(font: font_roman, lang: language, size: 12pt)
  ...
  if (theme == "lab") {
    v(1fr)
    align(center, image("./glo_img/ZJU-Banner.png", width: cover_image_size))
    align(center)[
      #set text(size: 26pt)
      #fakebold[本科实验报告]
    ]
    ...
    if (class != none) {
      rows.push([班$space.quad space.quad$级])
      rows.push(class)
    }
    if (place != none) {
      rows.push([实验地点])
    ...
    box(
      ...
      align(center)[
        ...
        #image("glo_img/ZJU-Logo.png", width: cover_image_size)
        #v(cover_image_padding)
    ...
    align(center)[
      #set text(size: 30pt)
      #set text(baseline: 15pt)  // 设置行间距为 10pt
